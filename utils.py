...
def check_is_correct(pred, gt, dataset, timeout=True):
    if dataset in ["triviaqa"]:
        for g in gt:
            if g.lower() in pred.lower():
                return True
        return False
    elif dataset in ["FEVER", "logieqa"]:
        if len(pred) > 0:
            ...
            return False
        return False
    elif dataset in ["medqa"]:
        answer = gt[1]
        choice = gt[0]
        if len(pred) > 0:
            if choice.lower() == pred.lower():
                return True
            else:
                return False
        else:
...
def extract_answer(pred_str, dataset=""):
